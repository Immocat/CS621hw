...
GlutExaminer::GlutExaminer(const char* _title, int _width, int _height)
...
void GlutExaminer::view_all() {
  translate(
      Vec3f(-float(modelview_matrix_[0] * center_[0] +
...
bool GlutExaminer::map_to_sphere(const Vec2i& _v2D, Vec3f& _v3D) {
  if ((_v2D[0] >= 0) && (_v2D[0] <= width_) && (_v2D[1] >= 0) &&
      (_v2D[1] <= height_)) {
    ...
    _v3D[2] =
        sinx2siny2 < 1.0 ? static_cast<float>(sqrt(1.0 - sinx2siny2)) : 0.0f;
...
void GlutExaminer::draw(const std::string& _draw_mode) {
  if (_draw_mode == "Wireframe") {
    ...
    std::cout << "This view mode is not supported for this geometry, you need "
                 "to load a mesh!"
              << std::endl;
  }
}
...
void GlutExaminer::translation(int x, int y) {
  ...
  float z = static_cast<float>(
      -((modelview_matrix_[2] * center_[0] + modelview_matrix_[6] * center_[1] +
         modelview_matrix_[10] * center_[2] + modelview_matrix_[14]) /
        (modelview_matrix_[3] * center_[0] + modelview_matrix_[7] * center_[1] +
  ...
  translate(Vec3f(2.0f * dx / float(width_) * right / near_ * z,
                  -2.0f * dy / float(height_) * up / near_ * z, 0.0f));
}
