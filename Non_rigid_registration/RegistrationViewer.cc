...
bool RegistrationViewer::open_meshes(
...
void RegistrationViewer::draw(int index, const OpenMesh::Vec3f &color) {
  ...
  glDrawElements(GL_TRIANGLES, indices_[index].size(), GL_UNSIGNED_INT,
                 &(indices_[index][0]));
...
void RegistrationViewer::keyboard(int key, int x, int y) {
  switch (key) {
    ...
    case 'n': {
      ...
      std::cout << "Process scan " << currIndex_ << " of "
                << int(meshes_.size()) << std::endl;
      glutPostRedisplay();
      break;
    }
    case 's': {
    ...
    case 'h': {
      ...
      printf("'n'\t-\tnext mesh\n");
      printf(
          "'r'\t-\tregister current mesh selected mesh using point-2-point "
          "optimization\n");
      printf(
          "' '\t-\tregister current mesh selected mesh using point-2-surface "
          "optimization\n");
...
std::vector<int> RegistrationViewer::subsample(
  ...
  // EXERCISE 2.2 /////////////////////////////////////////////////////////////
  // subsampling:
  // perform uniform subsampling by storing the indeces of the subsampled points
  // in 'indeces' (Hint: take advantage of the fact that consecutive points in
  // the
