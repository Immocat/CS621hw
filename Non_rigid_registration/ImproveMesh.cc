...
//   // when set_vertex_handle is called, the opposite half edge's from vertex
//   will
//   // set automatically
...
//   // }
//   // is eventList is valid, add new event to frame S_id - 2, since S_id is
//   point
  ...
  for (Mesh::EdgeIter e_it = mesh.edges_begin(); e_it != e_end; ++e_it) {
    ...
    if (edgeLength2 > max_edge_norm2) {
      ...
      if (eventList) {
        ...
        newEvent.end = vh.idx();
        newEvent.alpha[0] = 0.5;
        eventList->events_at_frame[S_id - 2].emplace_back(newEvent);
      }
